...
module braid_stable::stable_math {
    // Sui's prelude binds `vector` to `std::vector` for you; Aptos does not, so
    // the limit-cycle history below needs the import spelled out.
    use std::vector;
    ...
    /// ```
    ///
    /// with `D_P = D^(n+1) / (n^n prod(x))`, seeded at `D = S`. From that seed the
