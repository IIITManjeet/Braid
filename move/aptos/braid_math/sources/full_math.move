...
///
/// APTOS PORT. Transliterated from `move/sui/braid_math/sources/full_math.move`.
/// Every operation, every widening, and every division order is the same, because
/// floor division does not reassociate and the Rust replica in `node/crates` is
/// written against this exact sequence. The only edits are dialect-forced: Sui's
/// `let mut x` is spelled `let x` here, since Aptos Move has no `mut` on locals.
