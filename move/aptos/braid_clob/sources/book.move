...
module braid_clob::book {
    // Aptos has two table types: `table` (no length tracked) and
    // `table_with_length`. Sui's Table tracks its size, and `destroy_empty`
    // below needs that, so the with-length one is the honest analogue. Aliasing
    // it keeps the rest of this module character-for-character the Sui source.
    ...
    public fun empty(): Book {
        Book {
            bids: critbit::empty<Level>(),
            asks: critbit::empty<Level>(),
            orders: table::new(),
            next_order_id: 1,
        }
    }
    ...
    public fun quote(book: &Book, price: u64, quantity: u64, is_bid: bool): (u64, u128) {
        ...
        let leaf = if (is_bid) { critbit::min_leaf(side) } else { critbit::max_leaf(side) };
