    ...
    fun successors_and_predecessors_walk_the_whole_tree() {
        {
            ...
            let walked = vector<u64>[];
            let leaf = critbit::next_leaf(&tree, 0);
            ...
            let back = vector<u64>[];
            let leaf2 = critbit::prev_leaf(&tree, 999);
    ...
    fun traversal_agrees_with_a_linear_scan_from_every_probe() {
        ...
        // numbers hides that; probing everything does not.
        {
            ...
            while (probe <= 1100) {
                ...
                let expected_next: u64 = 0;
                let have_next = false;
                let expected_prev: u64 = 0;
                let have_prev = false;
                let i = 0;
                while (i < keys.length()) {
                    let k = keys[i];
